...
## 📋 목차
1. [프로젝트 배경 (Background)](#1-프로젝트-배경)
2. [기술 아키텍처 (Architecture)](#2-기술-아키텍처)
3. [핵심 알고리즘 (Core Logic)](#3-핵심-알고리즘)
4. [실험 환경 (Environment)](#4-실험-환경)
5. [실행 방법 (How to Run)](#5-실행-방법)
6. [실험 결과 (Benchmarks)](#6-실험-결과)
7. [트러블 슈팅 (Troubleshooting)](#7-트러블-슈팅)
8. [한계 및 향후 과제 (Roadmap)](#8-한계-및-향후-과제)
...
| Algorithm | Throughput (ops/s) | Load Stddev (표준편차) | Improvement |
|:---|:---:|:---:|:---:|
| Consistent Hashing | 159,608 | 725,757 | - |
| **D-HASH** | **159,927** | **531,824** | **🔻 26.7% 개선** |

**👉 분석**: 처리량(Throughput) 손실 없이 노드 간 부하 편차를 **26.7%** 줄여 시스템 안정성을 확보했습니다.
...
2.  **Demotion(강등) 로직 미구현**: 트래픽이 식은 핫키를 다시 일반 키로 돌리는 로직이 없어, 장기적으로 캐시 효율이 저하될 수 있습니다.
