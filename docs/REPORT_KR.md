...
* **문제**: 대규모 시나리오 시 MD5 연산의 높은 CPU 비용으로 처리량이 급감하고, Python 객체 오버헤드로 인한 메모리 부족 발생.
* **해결**: 암호화용이 아닌 고속 비암호화 해시 `xxHash64`로 교체하고, 클래스에 `__slots__`를 적용해 인스턴스 메모리 사용량 최적화.
* **결과**: **해싱 속도 20배 향상 및 메모리 50% 절감**으로 10만 개 이상의 키 테스트 환경 확보.
...
* **해결**: 전환 전후로 일정 기간 데이터를 양쪽 노드에 유지하는 **Guard Phase**와 비동기 사전 예열(Pre-warming) 전략 도입.
...
* **해결**: 읽기는 동적으로 분산하되, 쓰기는 해시 링이 지정한 주 노드(Primary)에 항상 고정하는 전략 채택.
* **결과**: 복잡한 분산 락 없이 **데이터 파편화 0%** 달성 및 시스템 복잡도 최소화.
...
* **해결**: `ThreadPoolExecutor`를 활용해 I/O를 병렬화하고, 노드별 버킷팅으로 락 경합을 최소화한 전용 벤치마크 툴 개발.
* **결과**: **180,000 OPS급 고부하 환경**을 구현하여 실제 운영 수준의 성능 검증 및 P99 지연 시간 정밀도 확보.
