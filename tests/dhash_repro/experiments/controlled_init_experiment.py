...
class ControlledInitExperiment:
    def setUp(self) -> None:
        ...
        trace_path = Path(__file__).resolve().parents[1] / "data" / "processed" / "nasa_trace.txt"
        ranked_keys, trace_size = self._load_nasa_trace(trace_path)
        ...
        print(
            f"[setup] Loaded trace from {trace_path} "
    ...
    @staticmethod
    def _load_nasa_trace(path: Path) -> Tuple[List[str], int]:
        ...
        if not counts:
            raise ValueError(f"NASA trace file is empty: {path}")
